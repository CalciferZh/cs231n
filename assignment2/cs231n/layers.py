...
def batchnorm_backward(dout, cache):
    ...
    # cache = (x, xhat, gamma, running_var, running_mean, eps)
    x, xhat, gamma, var, mean, eps = cache
    N = x.shape[0]
    
    dxhat = dout * gamma
    
    dvar = dxhat * (x-mean) * (-0.5) * (var + eps)**(-1.5)
    dvar = np.sum(dvar, axis=0)
    
    dmean = -dxhat / np.sqrt(var + eps) + dvar * (-2) * (x - mean) / N
    dmean = np.sum(dmean, axis=0)
    
    dx = dxhat / np.sqrt(var + eps) +  dvar * 2 * (x - mean) / N
    dx += dmean / N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    pad = conv_param['pad']
    # height and wight after padding
    N,C,H,W = x.shape
    # filter size
    F,C,HH,WW= w.shape
    # output size
    ...
    for p in range(N):
        for f in range(F):
            for i in range(outH):
                for j in range(outW):
                    field = pad_x[p, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
                    out[p,f,i,j] = np.sum(field * w[f]) + b[f]
...
def conv_backward_naive(dout, cache):
    ...
    for p in range(N):
        for f in range(F):
            for i in range(outH):
                for j in range(outW):
                    field = x[p, :, i*stride:i*stride+HH, j*stride:j*stride+WW]
                    dfield = dx[p, :, i*stride:i*stride+HH, j*stride:j*stride+WW]

                    dfield += dout[p,f,i,j] * w[f]
                    dw[f] += dout[p,f,i,j] * field
                    db[f] += dout[p,f,i,j]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for p in range(N):
        for i in range(outH):
            for j in range(outW):
                field = x[p, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
                out[p,:,i,j] = np.max(field, axis=(1,2))
    ###########################################################################
...
def max_pool_backward_naive(dout, cache):
    ...
    for p in range(N):
        for i in range(outH):
            for j in range(outW):
                field = x[p, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
                dfield = dx[p, :, i*stride:i*stride+pool_height, j*stride:j*stride+pool_width]
                maxi = np.max(field, axis=(1,2), keepdims=True)
                dfield += (field==maxi) * dout[p,:,i,j].reshape(C,1,1)
    ###########################################################################
